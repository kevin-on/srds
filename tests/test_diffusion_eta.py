#!/usr/bin/env python3
"""
Unit tests to verify that ddim_step_with_eta produces the same results as scheduler.step() with eta parameter.
"""
...
from diffusion import ddim_step_with_eta, _single_diffusion_step
    ...
    def test_ddim_step_with_eta_vs_scheduler_with_eta(self):
        ...
        for eta in eta_values:
            with self.subTest(eta=eta):
                ...
                latent_model_input = test_scheduler.scale_model_input(latent_model_input, self.timestep)
                ...
                if hasattr(test_scheduler, '_get_variance'):
                    ...
                    try:
                        ...
                        self.assertLess(max_diff, tolerance, 
                                       f"ddim_step_with_eta differs too much from scheduler.step() for eta={eta}")
                        print(f"✓ PASS: eta={eta} results match within tolerance {tolerance}")
                        
                    except TypeError:
                        # Fallback: scheduler.step() doesn't support eta parameter
                        print(f"⚠ SKIP: scheduler.step() doesn't support eta parameter")
                        self._test_manual_eta_implementation(eta, test_scheduler, noise_pred)
                        
                else:
                    print(f"⚠ SKIP: scheduler doesn't have _get_variance method")
                    self._test_manual_eta_implementation(eta, test_scheduler, noise_pred)
