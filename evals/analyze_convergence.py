...
def main():
    ...
    for exp_dir in exp_dirs:
        exp_name, convergence_result, l1_result, mse_result, gt_error_result = analyze_experiment_iterations(exp_dir)
        if convergence_result is not None or l1_result is not None or mse_result is not None or gt_error_result is not None:
    ...
    if results:
        ...
        for i, (exp_name, convergence_result, _, _, _) in enumerate(results):
            if convergence_result is not None:
                ...
                ax1.plot(valid_iterations, valid_means, 'o-', 
                        label=f'{legend_name}', color=colors[i], linewidth=2, markersize=4)
        ...
        for i, (exp_name, _, _, _, gt_error_result) in enumerate(results):
            if gt_error_result is not None:
                ...
                ax3.plot(valid_iterations, valid_means, 'o-', 
                        label=f'{legend_name}', color=colors[i], linewidth=2, markersize=4)
        ...
        for i, (exp_name, convergence_result, l1_result, mse_result, gt_error_result) in enumerate(results):
            exp_dir = exp_dirs[i]
            legend_name = extract_legend_name(exp_dir)
            ...
            if gt_error_result is not None:
                ...
                if len(valid_means) >= 2:
                    total_improvement = valid_means[0] - valid_means[-1]
                    avg_improvement_per_iter = total_improvement / (len(valid_means) - 1)
                    print(f"  GT Error - Initial: {valid_means[0]:.4f}, Final: {valid_means[-1]:.4f}")
