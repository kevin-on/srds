...
def get_all_images(prompt_dir):
    ...
    for pattern in iteration_patterns:
        ...
        for img_file in image_files:
            ...
            match = re.search(
                r"(?:srds_)?(?:sample_)?iteration_(\d+)\.png", os.path.basename(img_file)
            )
            if match:
...
def analyze_experiment_metric(exp_dir, metric, inferencer):
    ...
    for prompt_dir in prompt_dirs:
        ...
        scores = calculate_metric_for_iterations(all_images, prompt_text, metric, inferencer)
...
def main():
    ...
    for exp_dir in exp_dirs:
        exp_name, iteration_stats, final_scores, baseline_scores = analyze_experiment_metric(
            exp_dir, args.metric, inferencer
        )
