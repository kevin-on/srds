...
def save_key_metrics_summary(output_dir: str, algorithm: str):
    ...
    summary = {"algorithm": algorithm, "metrics": {}}
        ...
        try:
            ...
            if "Final_Image_Error" in df.columns:
                ...
                if len(errors) > 1:
                    ...
                    summary["metrics"]["avg_change_per_iteration"] = float(
            ...
            try:
                    ...
                    if "is_original:" in line:
                        ...
                        if "Timestep" in line and "Selected candidate" in line:
                            try:
                                ...
                                timestep = int(parts)
                                candidate_part = (
                                    line.split("Selected candidate")[1].split("(")[0].strip()
                                )
                ...
                if total_selections > 0:
                    summary["metrics"]["segment_selection"] = {
                        ...
                        "alternative_selection_rate": (total_selections - original_selections)
                        / total_selections,
    ...
    with open(text_summary_file, "w") as f:
        ...
        if "iteration_changes" in summary["metrics"]:
            f.write(
                f"Average Change per Iteration: "
                f"{summary['metrics']['avg_change_per_iteration']:.6f}\n"
            )
            f.write(
                f"Iteration Changes: "
                f"{[f'{c:.6f}' for c in summary['metrics']['iteration_changes']]}\n"
            )
        ...
        if algorithm == "sparareal" and "segment_selection" in summary["metrics"]:
            ...
            f.write("\nSegment Selection:\n")
            f.write(
                f"  Original selections: {sel['original_selections']}/"
                f"{sel['total_selections']} ({sel['original_selection_rate']:.1%})\n"
            )
            f.write(
                f"  Alternative selections: {sel['alternative_selections']}/"
                f"{sel['total_selections']} ({sel['alternative_selection_rate']:.1%})\n"
            )
