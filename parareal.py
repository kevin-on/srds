...
class Parareal:
    """
    Minimal class wrapper for the SRDS/Parareal-style diffusion run in this module.

    This lets you configure once and call run(prompts) multiple times, while
    keeping backward compatibility with the existing function API.
    """
    ...
    @torch.no_grad()
    def set_pipe(self, coarse_num_inference_steps, fine_num_inference_steps):
        ...
        self.scheduler_coarse = DDIMScheduler.from_pretrained(
            self.model_id, subfolder="scheduler", timestep_spacing="trailing"
        )
        self.scheduler_fine = DDIMScheduler.from_pretrained(
            self.model_id, subfolder="scheduler", timestep_spacing="trailing"
        )
        self.scheduler_coarse.set_timesteps(coarse_num_inference_steps)
        self.scheduler_fine.set_timesteps(fine_num_inference_steps)
        
        
        if not all(t in self.scheduler_fine.timesteps for t in self.scheduler_coarse.timesteps):
            raise ValueError("The coarse timesteps are not a subset of the fine timesteps")
        
        self.pipe_coarse = StableDiffusionPipeline.from_pretrained(
            self.model_id,
            scheduler=self.scheduler_coarse,
        )
        self.pipe_coarse = self.pipe_coarse.to(self.device)
        
        # Indices of coarse timesteps within fine timesteps; append end-exclusive boundary
        self.coarse_indices = torch.cat(
            [torch.where(self.scheduler_fine.timesteps == t)[0] for t in self.scheduler_coarse.timesteps]
        )
        self.coarse_indices = torch.cat(
            [self.coarse_indices, self.coarse_indices.new_tensor([len(self.scheduler_fine.timesteps)])]
        )

        print("SRDS Algorithm Configuration:")
        print(f"  Coarse steps: {len(self.scheduler_coarse.timesteps)} | timesteps: {self.scheduler_coarse.timesteps}")
        print(f"  Fine steps: {len(self.scheduler_fine.timesteps)} | timesteps: {self.scheduler_fine.timesteps}")
    
    @torch.no_grad()
    def encode_prompt(self, prompts, guidance_scale):
        
        do_classifier_free_guidance = guidance_scale > 1.0
        prompt_embeds, negative_prompt_embeds = self.pipe_coarse.encode_prompt(
            prompt=prompts,
            device=self.device,
    ...
    def solve_trajectory(
        ...
        """
        Return coarse trajectory solved with given scheduler
        """
        do_classifier_free_guidance = guidance_scale > 1.0
        trajectory_list = []
        ...
        for t in tqdm(scheduler.timesteps):
            if t in self.scheduler_coarse.timesteps:
                trajectory_list.append(latents.clone())
            latents = diffusion_step(
                latents,
                t,
                prompt_embeds,
                self.pipe_coarse.unet,
                scheduler,
                guidance_scale,
        ...
        return torch.cat(trajectory_list, dim=0)
    ...
    def solve_segment(
        ...
        return torch.cat([initial_latents.clone(), latents.clone()], dim=0)
    ...
    def run(
        ...
        # Set pipe and scheduler
        self.set_pipe(
            coarse_num_inference_steps=coarse_num_inference_steps, 
            fine_num_inference_steps=fine_num_inference_steps
        )
        
        # Encode prompt
        prompt_embeds = self.encode_prompt(prompts, guidance_scale)
        
        # Prepare latent variables
        num_channels_latents = self.pipe_coarse.unet.config.in_channels
        initial_latents = self.pipe_coarse.prepare_latents(  # line 1 of Algorithm 1
            len(prompts),
        ...
        tmp = self.solve_segment(
            initial_latents,
            prompt_embeds,
            guidance_scale,
            self.scheduler_coarse,
            self.scheduler_coarse.timesteps,
        )

        # Initialization
        prev_latents = torch.cat([initial_latents] * coarse_num_inference_steps, dim=0)  # prev_{i} in Algorithm 1
        cur_latents = prev_latents.clone()  # cur_{i} in Algorithm 1


    @torch.no_grad()
    def sanity_check_initialization(self):
        """
        Sanity check initialized latents
        Initialized latents at t = 0 should be the same as the that of the DDIM coarse output
        """
        coarse_output = self.pipe_coarse(
            prompts,
        ...
        initialized_images = decode_latents_to_pil(
            prev_latents[-len(prompts) :], self.pipe_coarse
        )
